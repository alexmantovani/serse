...
uuidAccredit = [0x09, 0x03, 0x01, 0x0A, 0x1C, 0x2D, 0xB1, 0x3E, 0xAC, 0x7A, 0x11, 0x3F, 0xCC, 0xE3, 0x44, 0x59]
...
def read_accredit_ed2(file_name):
    ...
    cks_compara = int.from_bytes(byte_array[ofs + 4:ofs + 8], byteorder='little')
    if cks != cks_compara:
        logga(f"CKS FAIL!!! {hex(cks)} != {cks_compara} delta {cks_compara-cks}    {byte_array[ofs :ofs + 8]}")
        logga(f"{hex(byte_array[ofs + 4])} {hex(byte_array[ofs + 5])} {hex(byte_array[ofs + 6])} {hex(byte_array[ofs + 7])} ")
        return None

    parserizza_qmap(byte_array[8:8 + ofs])
...
def create_accredit_ed2(username, user_id, pin='0000', machine='all', account_level="7", account_state=0, duration="7"):
    ...
    to_date = int(datetime.timestamp( datetime.now() + timedelta(days=int(duration))))
    ...
    accredit.append({ 'key': 'accountduration', 'value': int(duration)*24, 'type': 'Int' })
...
def write_accredit_ed2(file_name, accredit):
    ...
    info: bytes = b''
    info += (12).to_bytes(4, byteorder='little')  # TODO: DA CAPIRE cosa significa 12
...
def read_accredit(file_name) -> Accredit:
    ...
    accredit.accountLevel = int.from_bytes(byte_array[512:516], byteorder='big', signed=True)
    accredit.accountState = int.from_bytes(byte_array[516:520], byteorder='big', signed=True)
    accredit.accountExpiry = int.from_bytes(byte_array[520:524], byteorder='big', signed=True)
    accredit.accountSince = int.from_bytes(byte_array[524:528], byteorder='big', signed=True)
    accredit.accountUntil = int.from_bytes(byte_array[528:532], byteorder='big', signed=True)
    accredit.accreditLife = int.from_bytes(byte_array[532:536], byteorder='big', signed=True)
    # PIN
    accredit.PIN_0 = int.from_bytes(byte_array[536:540], "little", signed=False)
    accredit.PIN_1 = int.from_bytes(byte_array[540:544], "little", signed=False)
    accredit.PIN_2 = int.from_bytes(byte_array[544:548], "little", signed=False)
    accredit.PIN_3 = int.from_bytes(byte_array[548:552], "little", signed=False)
...
if len(sys.argv) < 3:
    ...
    logga(
        f"{sys.argv[0]} -username <username> -id <user id> -pin <pin> -token <token> -dest_dir <dest_dit> -type <ed1/ed2> [-machine [machine] -level [level]]")
    exit(11)
...
for item in sys.argv[1:]:
    if option is None:
        if item in ['-username', '-id', '-pin', '-machine', '-level', '-duration', '-dest_dir', '-token', '-type']:
            option = item
        else:
            logga(f"ERROR: '{item}' sconosciuto")
            exit(12)
    else:
        if option == '-username':
            parameters['username'] = item
        elif option == '-id':
            parameters['user_id'] = item
        elif option == '-pin':
            parameters['pin'] = item
        elif option == '-machine':
            parameters['machine'] = item
        elif option == '-level':
            parameters['account_level'] = item
        elif option == '-duration':
            parameters['duration'] = item
        elif option == '-token':
            output_file = item
        elif option == '-dest_dir':
            dest_dir = item
        elif option == '-type':
            display_type = item.lower()
            if display_type not in ['ed1','ed1.2','ed2']:
                logga(f"ERROR: 'Display type error")
                exit(13)
        option = None

if output_file is None:
    logga(f"ERROR: parametro '-token' non trovato")
    exit(14)

if dest_dir is None:
    logga(f"ERROR: parametro '-dest_dir' sconosciuto")
    exit(15)

# Creo una directory temporanea dove parcheggiare i files
with tempfile.TemporaryDirectory() as tmpdirname:
    # print('created temporary directory', tmpdirname)

    if display_type.lower() == 'ed2':
        accredit = create_accredit_ed2(**parameters)
        write_accredit_ed2(os.path.join(tmpdirname, "pendrive.accr"), accredit)
    else:
        accredit = create_accredit(**parameters)
        write_accredit(os.path.join(tmpdirname, "pendrive_accredit"), accredit)
        write_accredit(os.path.join(tmpdirname, "pendrive_accredit1"), accredit)

    dest_file = os.path.join(dest_dir, output_file)
    zippa_accredito(tmpdirname, dest_file)

    exit(0)

logga(f"ERROR: Errore durante la creazione della directory condivisa")
exit(20)
